(function () {
    'use strict;';
    // Select element
    const formElem = document.getElementById('add-new-form');
    const questionInput = document.getElementById('question');
    const answerInput = document.getElementById('answer');
    const triggerFormBtn = document.querySelector('.btn--trigger-form');
    const closeFormBtn = document.querySelector('.btn--close-form');
    const clearCardsBtn = document.querySelector('.btn--clear-cards');
    const cardsSlider = document.querySelector('.cards-slider-wrapper');
    const navPre = document.querySelector('.slider-prev');
    const navNext = document.querySelector('.slider-next');
    const cards = JSON.parse(localStorage.getItem('cards')) || [];
    let activeIndex = 1;
    // Validation form
    function showError(input) {
        const formGroup = input.parentElement;
        const errorElem = formGroup.querySelector('.error');
    ...
    function flipCard() {
        document.addEventListener('click', (e) => {
            ...
            if (!target.classList.contains('card__question') &&
    ...
    function moveSlider(oldIndex) {
        ...
        document.querySelector(`.card:nth-child(${oldIndex})`).className = `card ${oldIndex > activeIndex ? 'right' : 'left'}`;
